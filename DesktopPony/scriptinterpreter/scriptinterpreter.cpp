    ...
    try {
        ...
        if(node->m_p == parser_sl) {
            ...
            if(t.syn == syn_continue) {
                ...
                QVector<TokenData> t;
                t.append(t);
                throw new ScriptException(ScriptException::I015, t);
                ...
                QVector<TokenData> t;
                t.append(t);
                throw new ScriptException(ScriptException::I016, t);
            }
        }
        //根节点为表达式
...
TokenData ScriptInterpreter::iIf(ScriptParserNode *node, QMap<QString, QVector<QPair<int, Variable *> > *> *data, QVector<QVector<QString> *> *record, int layer)
{
    ...
    if(value.size() != 1) {
        ...
        i.append(1);
        i.append(value.size());
        throw new ScriptException(ScriptException::I001, t, i);
        ...
        QVector<TokenData> t;
        t.append(t);
        throw new ScriptException(ScriptException::I002, t, v);
    }
    try {
...
Variable *ScriptInterpreter::iDeclarator(TokenData type, ScriptParserNode *node, QMap<QString, QVector<QPair<int, Variable *> > *> *data, QVector<QVector<QString> *> *record, int layer)
{
    ...
    try {
        ...
        if(se == ScriptException::I009) {
            ...
            QVector<TokenData> t;
            t.append(node->m_t);
            throw new ScriptException(ScriptException::I009, t);
        } else if(se == ScriptException::I010) {
            /************
             * 异常
             * 重复成员
             ************/
            QVector<TokenData> t;
            t.append(node->m_t);
            throw new ScriptException(ScriptException::I010, t);
        }
    }
...
QVector<int> ScriptInterpreter::iDeclaratorSize(ScriptParserNode *node, QMap<QString, QVector<QPair<int, Variable *> > *> *data, QVector<QVector<QString> *> *record, int layer)
{
    ...
    if(v.isInt()) {
        ...
        vector.append(v);
        throw new ScriptException(ScriptException::I003, t, vector);
    }
...
void ScriptInterpreter::iInitializer(Variable *var, QVector<int> subscripte, ScriptParserNode *node, QMap<QString, QVector<QPair<int, Variable *> > *> *data, QVector<QVector<QString> *> *record, int layer)
{
    ...
    if(node->m_p_left_child->m_p == parser_ass_exp) {
        if(var->isArray() && (subscripte.size() != 1 || subscripte.first() != 0)) {
            ...
            QVector<TokenData> t;
            t.append(node->m_t);
            throw new ScriptException(ScriptException::I014, t);
        }
        try {
            Value v = iAssignmentExpression(node->m_p_left_child, data, record, layer);
            Value *pv;
            if(var->isArray()) {
...
void ScriptInterpreter::iInitializerInitElement(Value *var, Value value)
{
    ...
    if(var->type == type_str) {
        if(value.type == type_str) {
            ...
            throw ScriptException::I009;//异常：无法初始化变量
        }
    }
    //整型
    ...
    //布尔型
        ...
        throw ScriptException::I002;//异常：无效的运算
    }
}
...
Value ScriptInterpreter::iAssignmentExpression(ScriptParserNode *node, QMap<QString, QVector<QPair<int, Variable *> > *> *data, QVector<QVector<QString> *> *record, int layer)
{
    ...
    //          -> UEXP |= AEXP
        ...
        if(left.type != type_var) {
            ...
            QVector<TokenData> t;
            t.append(node->m_t);
            throw new ScriptException(ScriptException::I008, t, v);
        }
    ...
    if(node->m_p_left_child->m_p == parser_prim_exp) {
        ...
        if(left.type == type_id) {
            ...
            try {
                ...
                if(se == ScriptException::I002) {
                    ...
                    t.append(t);
                    throw new ScriptException(ScriptException::I002, t, v);
                } else if(se == ScriptException::I011) {
                    /******************
                     * 异常
                     * 数组索引超出数组结尾
                     ******************/
                    ...
                    QVector<TokenData> t;
                    t.append(t);
                    throw new ScriptException(ScriptException::I011, t, v);
                } else if(se == ScriptException::I012) {
                    /**************
                     * 异常
                     * 下标值不是数组
                     **************/
                    ...
                    QVector<TokenData> t;
                    t.append(t);
                    throw new ScriptException(ScriptException::I012, t, v);
                }
            }
        }
        return left;
        ...
        if(left.type != type_var) {
            if(node->m_d.syn == syn_inc_op) {
                ...
                QVector<Value> v;
                v.append(left);
                throw new ScriptException(ScriptException::I006, t, v);
            } else {
                /******************
                 * 异常
                 * 无法减少指定类型的值
                ...
                QVector<Value> v;
                v.append(left);
                throw new ScriptException(ScriptException::I007, t, v);
            }
        }
        Value v = *left._var;
        try {
...
Value ScriptInterpreter::iPrimaryExpression(ScriptParserNode *node, QMap<QString, QVector<QPair<int, Variable *> > *> *data, QVector<QVector<QString> *> *record, int layer)
{
    ...
    //文法：PRIMEXP -> ID
    else if(node->m_d.syn == syn_str_lit) {
        try {
...
Variable *ScriptInterpreter::insertVariable(QString id, TokenData type, QMap<QString, QVector<QPair<int, Variable *> > *> *data, QVector<QVector<QString> *> *record, int layer)
{
    ...
    if(data->contains(id)) {
        ...
        if(vector->last().first == layer) {
            throw ScriptException::I010;//异常:重复成员
        } else {
...
Value ScriptInterpreter::getVariable(QString id, QVector<int> subscripte, QMap<QString, QVector<QPair<int, Variable *> > *> *data)
{
    ...
    if(var->isArray() == false) {
        throw ScriptException::I012;//异常：下标值不是数组
    }
    //判断下标是否越界
    if(subscripte.size() >= var->getSize().size()) {
        throw QPair<int, ScriptException::EXCEPTION_TYPE>(var->getSize().size(), ScriptException::I011);//异常：数组下标超出数组结尾
    }
...
QVector<int> ScriptInterpreter::iVariableSubscripte(ScriptParserNode *node, QMap<QString, QVector<QPair<int, Variable *> > *> *data, QVector<QVector<QString> *> *record, int layer)
{
    ...
    if(v.first().isInt()) {
        ...
        QVector<Value> v;
        v.append(v.first());
        throw new ScriptException(ScriptException::I003, t, v);
    }
