...
{
    m_token = new QVector<QPair<QString, SYN>>;
}

QVector<QPair<QString, SYN>>* ScriptLexer::divToken(QString str)
{
    m_str = str;
    m_point = -1;
    while(m_point < m_str.size())
    {
        m_token_num.clear();
        SYN syn = judge();
        QPair<QString, SYN> token(m_token_num, syn);
...
SYN ScriptLexer::judge()
{
    ...
    if((m_ch>='a' && m_ch <= 'z') || (m_ch >= 'A' &&m_ch <= 'Z') || m_ch == '_'){
        ...
        m_token_num += m_ch;
        switch(m_ch.cell()){
        case ';': return syn_semi;
        case '{': return syn_front_body;
        case '}': return syn_back_body;
        case ',': return syn_comma;
        case '(': return syn_front_bracket;
        case ')': return syn_back_bracket;
        case '[': return syn_front_array;
        case ']': return syn_back_array;
        case '?': return syn_query;
        case '.':
            if(get(1) == '.')
            {
                if(get(2) == '.')
