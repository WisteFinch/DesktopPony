...
{
    node->set(nullptr, parser_s, nullptr, nullptr);
    //分析文法
    //获取当前单词
    TokenData token = curToken();
...
{
    node->set(nullptr, parser_cs, nullptr, nullptr);
    //分析文法
    //获取当前单词
    TokenData token = curToken();
...
void ScriptParser::pIfBody(ScriptParserNode *node)
{
    ...
    node->set(nullptr, parser_if_body, nullptr, nullptr);
    //分析文法
    node->m_p_left_child = new ScriptParserNode();
    pStatement(node->m_p_left_child);
    TokenData token = curToken();
...
{
    node->set(nullptr, parser_declaration, nullptr, nullptr);
    //分析文法
    node->m_p_left_child = new ScriptParserNode();
...
{
    node->set(nullptr, parser_init_declarator_list, nullptr, nullptr);
    //分析文法
    node->m_p_left_child = new ScriptParserNode();
...
{
    node->set(nullptr, parser_init_declarator, nullptr, nullptr);
    //分析文法
    node->m_p_left_child = new ScriptParserNode();
...
{
    node->set(nullptr, parser_declarator, nullptr, nullptr);
    //分析文法
    node->m_p_left_child = new ScriptParserNode();
...
{
    node->set(nullptr, parser_declarator_size, nullptr, nullptr);
    //分析文法
    eat(syn_front_array);
    node->m_p_left_child = new ScriptParserNode();
...
void ScriptParser::pDeclarationTypeSpecifier(ScriptParserNode *node)
{
    ...
    //获取下一个单词
    TokenData token = getToken();
    node->set(token.str, parser_declaration_type_specifier, nullptr, nullptr, token);
}

void ScriptParser::pInitializer(ScriptParserNode *node)
{
    node->set(nullptr, parser_init, nullptr, nullptr);
...
{
    node->set(nullptr, parser_init_list, nullptr, nullptr);
...
void ScriptParser::pId(ScriptParserNode *node)
{
    ...
    //获取下一个单词
    TokenData token = getToken();
    node->set(token.str, parser_id, nullptr, nullptr, token);
}
...
{
    node->set(nullptr, parser_unary_exp, nullptr, nullptr);
    //分析文法
    TokenData token = curToken();
    node->m_p_left_child = new ScriptParserNode();
...
void ScriptParser::pConstant(ScriptParserNode *node)
{
    ...
    TokenData token = getToken();
    if(token.syn == syn_const_int) {
        node->set(token.str, parser_constant_int, nullptr, nullptr, token);
    } else if(token.syn == syn_const_float) {
        node->set(token.str, parser_constant_float, nullptr, nullptr, token);
    } else if(token.syn == syn_const_bool) {
        node->set(token.str, parser_constant_bool, nullptr, nullptr, token);
    } else if(token.syn == syn_const_char) {
        node->set(token.str, parser_constant_char, nullptr, nullptr, token);
    } else {
        errorExit();
    }
}
...
void ScriptParser::pStringLiteral(ScriptParserNode *node)
{
    ...
    //获取下一个单词
    TokenData token = getToken();
    node->set(token.str, parser_string_literal, nullptr, nullptr, token);
}

void ScriptParser::pVariableSubscripte(ScriptParserNode *node)
{
    node->set(nullptr, parser_variable_subscripte, nullptr, nullptr);
    //分析文法
    eat(syn_front_array);
    node->m_p_left_child = new ScriptParserNode();
    pExpression(node->m_p_left_child);
